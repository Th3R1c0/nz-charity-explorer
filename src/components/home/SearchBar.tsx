...
export const SearchBar = () => {
  ...
  return <div className="w-full relative mb-8" ref={searchContainerRef}>
    ...
    <div className={`
      ...
      <button
        ...
        className={`bg-primary text-primary-foreground w-9 h-9 md:w-10 md:h-10 rounded-full font-medium hover:bg-primary/90 transition-all flex items-center justify-center flex-shrink-0 ${query ? 'opacity-100' : 'opacity-0 md:opacity-100 pointer-events-none md:pointer-events-auto'
          }`}
      >
    ...
    {showResults && (isFocused || searchResults.length > 0) && <div className="absolute top-full left-0 right-0 mt-2 bg-background border border-border rounded-2xl shadow-2xl overflow-hidden z-50 max-h-[400px] md:max-h-[500px] overflow-y-auto">
      {isLoading || isTyping ? <div className="px-4 md:px-6 py-6 md:py-8 flex items-center justify-center text-muted-foreground text-sm md:text-base">
        <Loader2 className="w-4 h-4 md:w-5 md:h-5 animate-spin mr-2" />
        Searching...
      </div> : searchResults.length === 0 ? <div className="px-4 md:px-6 py-6 md:py-8 text-center text-muted-foreground text-sm md:text-base">
        ...
        {searchResults.map((charity, index) => {
          ...
          return <div key={charity.CharityRegistrationNumber + index} className={`flex items-start md:items-center gap-3 md:gap-4 px-3 md:px-5 py-3 md:py-4 transition-colors ${isDeregistered ? "opacity-50 cursor-not-allowed bg-muted/30" : "hover:bg-muted/50 cursor-pointer"}`} onClick={() => !isDeregistered && navigate(`/charity/${slug}`)}>
            {/* Left Column: Icon */}
            ...
            <div className="flex-1 min-w-0">
              ...
              {charity.CharitablePurpose && <p className="text-xs md:text-sm text-muted-foreground/70 mt-1 line-clamp-1 hidden md:block">
            ...
            <div className="flex-shrink-0">
              {charity.RegistrationStatus === "Registered" ? <div className="flex items-center gap-1 md:gap-1.5 px-2 md:px-3 py-1 md:py-1.5 rounded-full bg-emerald-500/10 text-emerald-600 text-[10px] md:text-xs font-medium">
                <CheckCircle2 className="w-3 h-3 md:w-3.5 md:h-3.5" />
                <span className="hidden sm:inline">Registered</span>
              </div> : <div className="flex items-center gap-1 md:gap-1.5 px-2 md:px-3 py-1 md:py-1.5 rounded-full bg-rose-500/10 text-rose-600 text-[10px] md:text-xs font-medium">
