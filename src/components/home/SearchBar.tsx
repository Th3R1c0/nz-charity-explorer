...
export const SearchBar = () => {
  ...
  return <div className="w-full relative mb-8" ref={searchContainerRef}>
      ...
      {showResults && (isFocused || searchResults.length > 0) && <div className="absolute top-full left-0 right-0 mt-2 bg-background border border-border rounded-2xl shadow-2xl overflow-hidden z-50 max-h-[400px] md:max-h-[500px] overflow-y-auto">
          ...
          return <div key={charity.CharityRegistrationNumber + index} className={`flex items-start md:items-center gap-3 md:gap-4 px-3 md:px-5 py-3 md:py-4 transition-colors ${isDeregistered ? "opacity-50 cursor-not-allowed bg-muted/30" : "hover:bg-muted/50 cursor-pointer"}`} onClick={() => !isDeregistered && navigate(`/charity/${charity.CharityRegistrationNumber}`)}>
                  {/* Left Column: Icon */}
